...
from Transformations import apply_translation, apply_reduce_to_3_features, apply_randomrotations_3_features
...
from Model_3_features import use_GPU, PairModel1, Branch
...
# The code is very similar to Mian.ipynb. The only differences are that 3 features are used, so there is the application of the apply_reduce_to_3_features() function at the beginning of the training cycle that extracts the first 3 features from the data. Also the apply_randomrotations_3_features() function is used, since apply_randomrotations() requires at least 6 features.
...
W_stored = torch.load(r'C:\\Users\\Alessandro\\Desktop\\Weights\\Main3_epoch63.pt')
...
for epoch in range(num_epochs):
    ...
    for batch_data in progress_bar:
        ...
        frags_a= apply_reduce_to_3_features(frags_a)
        frags_b= apply_reduce_to_3_features(frags_b)

        # Random rotations
        frags_a = apply_randomrotations_3_features(frags_a)
        frags_b = apply_randomrotations_3_features(frags_b)
    ...
    with torch.no_grad():
        for val_batch in val_loader_basic:
            ...
            # Reduce to three features
            val_frags_a= apply_reduce_to_3_features(val_frags_a)
            val_frags_b= apply_reduce_to_3_features(val_frags_b)
